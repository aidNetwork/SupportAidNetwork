...
export default function Mobility() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.idCard) {
        const idCardPath = `mobility-idcards/${Date.now()}-${formData.idCard.name}`;
        const { error: uploadError } = await supabase.storage.from('uploads').upload(idCardPath, formData.idCard);
