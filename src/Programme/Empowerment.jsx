...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const idCardPath = `idCards/id-${timestamp}-${idCardFile.name}`;
    const resumePath = `resumes/resume-${timestamp}-${resumeFile.name}`;

    // Upload ID Card
    const { data: idCardUpload, error: idCardError } = await supabase.storage
      .from('uploads')
    ...
    // Get public URLs
    const { data: idCardUrlData } = supabase
      .storage
      .from('uploads')
      .getPublicUrl(idCardPath);

    const { data: resumeUrlData } = supabase
      .storage
      .from('uploads')
      ...
      .insert([
        {
          ...
          idCardUrl: idCardUrlData.publicUrl,
